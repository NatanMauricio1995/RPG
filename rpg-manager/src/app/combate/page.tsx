...
const[personagensSelecionados,setPersonagensSelecionados]=useState<(string|number)[]>([]);
const[monstrosSelecionados,setMonstrosSelecionados]=useState<(string|number)[]>([]);
const[quantidadesMonstros,setQuantidadesMonstros]=useState<Record<string|number,number>>({});
...
useEffect(() => {
  if (user) {
    const carregar = async () => {
      const ps = await listarPersonagens(user.uid);
      const ms = await listarMonstrosCombate();
      setPersonagens(ps);
      setMonstros(ms);
    };
    carregar();
...
id:string|number,
lista:(string|number)[],
setLista:(valor:(string|number)[])=>void
){
...
const aliados=(personagens as EntidadeSelecionavel[]).filter((personagem)=>personagensSelecionados.includes(personagem.id));
const inimigos=monstros
.filter((monstro:EntidadeSelecionavel)=>monstrosSelecionados.includes(monstro.id))
.map((monstro:EntidadeSelecionavel)=>({
...monstro,
quantidade:quantidadesMonstros[monstro.id] || 1
}));
